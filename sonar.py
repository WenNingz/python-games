...
def drawBoard(board):
    ...
    # print the number across the bottom
    print()
    print('    ' + ('0123456789' * 6))
    print(hline)
...
def makeMove(board, chests, x, y):
    # Change the board data structure with a sobar device character. Remove treasure chests
    ...
    for cx, cy in chests:
        ...
        if distance < smallestDistance: # the closest treasure chest
...
def enterPlayerMove():
    # Let the player type in her move. Return a two-item list if int xy coodinates
...
To play, enter the coodinates of the point in the ocean you wish to drop a
...
The point whre the device was dropped will be marked with a 2.
...
If a device is directly dropped in a treasure chest, you have discovered
the location if the chest, and it will be collected . The sonar device will
remain there.
...
print('S O N A R !')
print()
print('Would you like to biew the instruction? (yes or no)')
...
while True:
    ...
    while sonarDevices > 0:
        ...
        x, y = enterPlayerMove()
        previousMoves.append([x, y]) # must track all moves so that sonar devices can be updated.
    ...
    if sonarDevices == 0:
        ...
        print('    The remaining cheset were here:')
